...
public class OzTtTpFpController extends BaseController {
	...
	public String surechangepw(Model model, HttpServletRequest request, HttpSession session, @ModelAttribute OzTtTpFpDto ozTtTpFpDto) {
		try{
	...
	public String sendMail(Model model, HttpServletRequest request, HttpSession session, @ModelAttribute OzTtTpFpDto ozTtTpFpDto) {
		try{
			// 先通过email拿到客户号
			...
			if (tCustomerLoginInfo != null) {
				...
				sendMailDto.setTitle("修改密码");
				StringBuffer sb = new StringBuffer();
				sb.append("点击下面地址的链接修改密码");
				sb.append("</br>");
				// 这里将客户号加密
				sb.append("http://localhost:8080/OZTT/OZ_TT_TP_FP/changepw/" + UrlParamDecode.paramEncode(customerNo));
